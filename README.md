...
詳細なハードウェア要件は [docs/hardware-guide.md](docs/hardware-guide.md) を参照してください。

## システム診断

**セットアップ前に、お使いの環境が要件を満たしているか確認できます。**

> 前提条件: Python 3.8以上がインストールされていること

### 環境チェック（Docker起動前でもOK）

ハードウェアスペックとDocker環境を診断します：
...
出力例:
  ...
  判定: 本番環境として推奨スペックを満たしています
```

スペックが不足している場合は、[docs/hardware-guide.md](docs/hardware-guide.md) のトラブルシューティングを参照してください。

### ベンチマーク（Docker起動後）

実際の処理速度を測定し、データ量ごとの処理時間を予測します。

> 前提条件: Dockerサービスが起動済み、`pip install requests` 実行済み

```bash
# まずはクイックテストで動作確認（1-2分）
...
| ドキュメント | 内容 |
|-------------|------|
| [クイックスタート](docs/quickstart.md) | 最速で立ち上げるためのステップバイステップガイド |
| [ユーザーズマニュアル](docs/user-manual.md) | 詳細な操作方法と全機能の説明 |
| [API リファレンス](docs/api-reference.md) | REST API の仕様と使用例 |
| [設定ガイド](docs/configuration.md) | 環境変数と設定オプションの詳細 |
